...
public class Bat2 : Enemy
{
    ...
    private void Start()
    {
        currentHp = maxHp;
        InitBehaviourTree();
        GameManager.instance.blackBoard.SetValue("Bat2", this);
        SortedSelectorNode mainSelector = new SortedSelectorNode("MainSelector");

        Leaf patrolStrategy = new Leaf("PatrolStrategy", new PatrolStrategyTwo(patrolPoints),20);
        ...
        Leaf attackCondition = new Leaf("AttackCondition", new ConditionTwo(() =>
        { 
            return attackProb > 60 && !playerController.isDead;
        }));

        Leaf attackStrategy = new Leaf("AttackStrategy", new AttackStrategyTwo());

        Leaf turnBackStrategy = new Leaf("TurnBackStrategy", new TurnBackToPatrolStrategyTwo(patrolPoints));
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(collision.TryGetComponent<PlayerController>(out PlayerController _controller))
        {
            ...
            {
                _controller.OnDamage(InflictDamage(13f));
